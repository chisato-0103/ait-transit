...
| `updated_at` | string | ○ | `+09:00` オフセット付きISO8601。保存時にサーバーが付与 |
...
- 保存API（PUT）の成功時に `clearDiaOverridesCache()` を呼ぶ。
  同一インスタンスでは即時、別インスタンスでも最大30秒で反映される

取得失敗時はビルド同梱の静的import値にフォールバックし、`source: "fallback"` を返す。
...
- セクション上部に実効値と取得状態を表示
  - 「本日のダイヤ: A → B（臨時）」
  - 「取得元: GitHub / 最終取得: 08:12:33」。`fallback` のときは警告色で表示する
...
| `PUT` | 全件置換。検証 → `saveDataFile()` → `clearDiaOverridesCache()` |
...
| 状況 | 挙動 |
|---|---|
| GitHub取得失敗・404・タイムアウト | ビルド同梱JSONにフォールバックし `source: "fallback"`、`console.warn` |
| JSONパース失敗 | 同上 |
