...
6. ✅ シャトル臨時ダイヤの手動上書き（2026-09-07完了）
   - 管理画面から日付単位でダイヤ種別を差し替えられる。
     holiday を指定するとその日は全便運休になる
   - 上書きは shuttle_dia_overrides.json に分離して保存するため、
     公式データを再取得して全面置換しても消えない
   - 本番はGitHub Contents APIから30秒TTLで読み直すので、
     再デプロイを待たずに反映される
   - 仕様書:
...
10. ✅ 鉄道の平日/土休日判定を路線別の暦判定に修正（2026-09-24完了）
   - 旧実装は「シャトルがAなら平日」で判定し、リニモ53日・愛環98日（FY2026）を誤判定していた
   - リニモ: 土日祝と8・9・2・3月の平日が土休日ダイヤ（linimo.jp のオレンジ時刻注記）
   - 愛環: 土日祝が土休日ダイヤ（全体時刻表の「土・休日運休」。学校休業期間は無関係）
   - 祝日は内閣府CSVから scripts/build-japanese-holidays.mjs で japanese_holidays.json を生成。
     運行カレンダーの年度更新時に翌年分があるか確認する（テストで検知）
   - 年末年始は公式記載がないため暦どおり。臨時ダイヤは各社告知を見て対応する
11. ✅ トップページのシャトルバス検索対策（2026-09-25完了）
   - 初期HTMLが「読み込み中...」だけだったため、サーバー描画の
     シャトルバス時刻表区画（折りたたみ・全269便）をお知らせの下に追加
   - ダイヤ名は公式表記の A/B/C のみ（公式に意味の説明がないため）
